...
import { SET_GAME_ID, SET_ROOM_ID } from '../redux/constants/game.constants'
...
const MenuPage = () => {
  ...
  const { createOffer, createAnswer, setRemoteAnswer } = useContext(PeerContext);

  const StartGameHandler = () => {
    dispatch(startGame({
      userId: user?.user?._id, email: user?.user?.email, roomId
    }))
  ...
  const HandleCreateNewRoom = () => {
    dispatch(createRoom({ socketId: socket.id }))
  }
  ...
  const HandleNewUserJoined = useCallback(async (data) => {
    dispatch(modifyplayerList(data.users))
    const offer = await createOffer();
    socket.emit('call-user', { email: data?.email, offer });
  }, [createOffer, socket])
  ...
  const HandleAcceptedCall = useCallback(async (data) => {
    ...
    console.log('call got accepted ', data?.ans)
  }, [socket])
          ...
          <div className="w-full max-w-md p-6 bg-gray-800/90 rounded-xl shadow-2xl border border-gray-700 animate-slide-up">
            ...
            <div className="my-4">
              {players?.map((player) => (
                ...
                >
                  {player?.email} { player?.isReady ? 'ready' : 'not ready' }
                </p>
              ))}
            </div>
            ...
            <div className="flex gap-4 justify-center mt-6">
              ...
              <button
                ...
                className={`py-2 px-6 ${StartGameSuccess
                  ? "bg-gradient-to-r from-blue-500 to-blue-700"
                  : "bg-gradient-to-r from-green-500 to-green-700"
                  } hover:from-${StartGameSuccess ? "blue" : "green"}-600 hover:to-${StartGameSuccess ? "blue" : "green"
